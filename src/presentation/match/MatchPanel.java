...
public class MatchPanel extends JPanel {
	...
	private void setComponentPosition() {
		...
		datePanel.setBounds(NUMBER.FRAME_WIDTH / percent, 0, NUMBER.FRAME_WIDTH * (percent - 2) / percent,
				(NUMBER.FRAME_HEIGHT - NUMBER.NAVIGATION_PANEL_HEIGHT) / percent);

		dateLabel.setBounds(datePanel.getWidth() / 6, datePanel.getHeight() / 4, NUMBER.FRAME_WIDTH * (percent - 2) / percent / 8,
				(NUMBER.FRAME_HEIGHT - NUMBER.NAVIGATION_PANEL_HEIGHT) / percent / 2);

		dateSelectPanel.setBounds(dateLabel.getX() + dateLabel.getWidth(), datePanel.getHeight() / 4, NUMBER.FRAME_WIDTH * (percent - 2) / percent
				/ 7, (NUMBER.FRAME_HEIGHT - NUMBER.NAVIGATION_PANEL_HEIGHT) / percent / 2);

		matchPanel.setBounds(datePanel.getX(), datePanel.getHeight(), datePanel.getWidth(), (this.getHeight() - datePanel.getHeight())
				* (percent - 2) / percent);
		...
		// switchToLastDay.setBounds(0, thisDateLabel.getHeight(),
		// this.getWidth()/percent, this.getHeight());

		// switchToNextDay.setBounds(this.getWidth()*(percent-1)/percent,
		// thisDateLabel.getHeight(), this.getWidth()/percent,
		// this.getHeight());
	...
	void initMatchInfo() {
		...
		for (int i = 0; i < numOfMatch; i++) {
			...
			// eachMatchLabel.setBounds(0,50*(i+1),this.getWidth()-switchToLastDay.getWidth()*2,
			// 80);
	...
	public static void main(String[] args) {
		...
		frame.setBounds((NUMBER.SCREEN_WIDTH - NUMBER.FRAME_WIDTH) / 2, (NUMBER.SCREEN_HEIGHT - NUMBER.FRAME_HEIGHT) / 2 - 20, NUMBER.FRAME_WIDTH,
				NUMBER.FRAME_HEIGHT);
		frame.setBackground(MyColor.MY_RED);
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.setVisible(true);

		Container contentPane = frame.getContentPane();
		contentPane.setLayout(null);
		contentPane.setBackground(MyColor.LIGHT_COLOR);
